...
async def create_campaign(campaign: SocialMediaCampaign, current_user: dict = Depends(get_current_user)):
    try:
        ...
        # Generate campaign content using MetaAI and ensure it's a string
        response = ai.prompt(ai_prompt)
        campaign_content = response.get('message') if isinstance(response, dict) else str(response)

        # Save campaign to the database
        campaign_dict = campaign.dict()
        campaign_dict['content_plan'] = campaign_content
        campaign_dict['created_at'] = datetime.utcnow()
        campaign_dict['user_id'] = str(current_user['_id'])

        result = await db['social_media_campaigns'].insert_one(campaign_dict)
        ...
        return SocialMediaCampaign(**created_campaign)
