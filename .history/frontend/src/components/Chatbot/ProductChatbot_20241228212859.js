    ...
    try {
        const chatHistory = {
            ...
            messages: [...messages, userMessage].map(msg => ({
                ...
                message: typeof msg.message === 'object' ? msg.message.intro : msg.message
            }))
        };

        await chatWithProduct(chatHistory, (response) => {
            const formattedResponse = formatBotResponse(response);
            setMessages(prev => {
                // Remove any existing bot response that's still streaming
