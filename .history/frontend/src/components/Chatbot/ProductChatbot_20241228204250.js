  ...
  const handleSend = async (e) => {
    ...
    try {
        ...
        if (response && response.messages) {
            ...
            const botMessage = { 
                sender: 'bot', 
                message: response.content || 'I apologize, but I encountered an issue processing your request.'
            };
            setMessages(prev => [...prev, botMessage]);
        }
    } catch (err) {
        console.error('Chat error:', err);
        const errorMessage = err.response?.data?.detail || 'Failed to communicate with the bot';
