...
const ImageEditor = () => {
    ...
    const handleGenerateBackground = async (prompt) => {
        try {
            ...
            // Assuming the response contains the image URL in base64 or direct URL format
            const imageUrl = response.imageUrl || response; // Adjust based on your API response

            return new Promise((resolve, reject) => {
                const img = new Image();
                img.crossOrigin = "Anonymous"; // Add this if loading from external URL
                
                img.onload = () => {
                    setBackground(img);
                    // Update or add the background layer
                    setLayers(prev => {
