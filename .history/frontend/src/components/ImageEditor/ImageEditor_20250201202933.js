...
const ImageEditor = () => {
    ...
    const handleGenerateBackground = async (prompt) => {
        if (!prompt || prompt.trim().length < 3) {
            setError('Please enter a valid prompt (at least 3 characters)');
            return;
        }
        
        try {
            setIsLoading(true);
            const generatedBg = await generateBackground(prompt);
            const img = new Image();
            img.src = generatedBg;
            img.onload = () => setBackground(img);
        } catch (err) {
            setError('Failed to generate background. Please try a different prompt.');
        } finally {
            setIsLoading(false);
        }
    };
