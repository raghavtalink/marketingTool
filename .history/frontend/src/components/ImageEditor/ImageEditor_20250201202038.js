...
const MemoizedTransformer = React.memo(() => (
...
const ImageEditor = () => {
    ...
    useEffect(() => {
        ...
        const handleTransform = throttle(() => {
        ...
        if (transformerNode && selectedId) {
            transformerNode.on('transform', handleTransform);
        }

        return () => {
            if (transformerNode) {
                transformerNode.off('transform', handleTransform);
            }
        };
    }, [selectedId]);
                            ...
                            {optimizedTextElements.map((text) => {
                                ...
                                return (
                                    <MemoizedText
                                        ...
                                        id={text.id}
                                        {...text}
                                        draggable={!text.isEditing && !layers.find(l => l.id === text.id)?.locked}
                                        onClick={(e) => {
                                            e.cancelBubble = true;
                                            setSelectedId(text.id);
                                            handleTextClick(text.id);
