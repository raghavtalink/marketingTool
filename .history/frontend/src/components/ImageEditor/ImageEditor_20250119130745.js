...
const ImageEditor = () => {
    ...
    const handleTextAdd = (e) => {
        ...
        const newTextId = handleTextManipulation.add(position, setTextElements, setLayers);
        setSelectedId(newTextId);
    };

    const handleTextDblClick = (e) => {
        const id = e.target.id();
        if (!id?.startsWith('text-')) return;
        
        const textarea = handleTextManipulation.startEditing(id, textElements, stageRef);
        
        textarea.addEventListener('blur', () => {
            handleTextManipulation.update(id, { text: textarea.value }, textElements, setTextElements);
                        ...
                        <Layer>
                            ...
                            {textElements.map(text => (
                                <Text
                                    ...
                                    id={text.id}
                                    {...text}
                                    draggable={tool === 'move' && !layers.find(l => l.id === text.id)?.locked}
                                    visible={layers.find(l => l.id === text.id)?.visible}
                                    onClick={() => setSelectedId(text.id)}
                                    onTransform={(e) => {
                                        const node = e.target;
                                        handleTextManipulation.update(
                                            text.id,
                                            {
