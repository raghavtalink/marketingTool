...
const ImageEditor = () => {
    ...
    const handleRemoveBackground = async () => {
        ...
        try {
            setIsLoading(true);
            console.log('Original Product Image:', productImage.src.src);
            
            const formData = new FormData();
            const blob = await fetch(productImage.src.src).then(r => r.blob());
            formData.append('image', blob, 'product.png');

            const result = await removeBackground(formData);
            console.log('Processed Image Data:', result.image.slice(0, 50));
            
            const img = new Image();
            img.crossOrigin = 'Anonymous';
            
            img.onload = () => {
                console.log('Processed Image Dimensions:', img.width, 'x', img.height);
