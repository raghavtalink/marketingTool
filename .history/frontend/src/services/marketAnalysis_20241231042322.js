  ...
  export const analyzeTrends = async (productId, trendType, timeframe) => {
    try {
      ...
      if (response.data.analysis) {
        ...
        // Create trend data points from the analysis
        const trends = [
          { label: 'Market Position', value: 1 },
          { label: 'Consumer Interest', value: 1 },
          { label: 'Price Competitiveness', value: 1 },
          { label: 'Growth Potential', value: 1 }
        ];
  
        return {
          trends,
          insights: insights.length > 0 ? insights : ['No market insights available']
        };
      }
