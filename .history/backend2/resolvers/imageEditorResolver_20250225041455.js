...
const processUpload = async (imageData) => {
  try {
    ...
    // Handle file upload stream (original functionality)
    if (imageData.createReadStream) {
      const { createReadStream } = imageData;
  ...
  Mutation: {
    removeBackground: async (_, { input }, { user }) => {
      ...
      try {
        ...
        console.log(`[removeBackground] Processed image size: ${processedBuffer.length} bytes`);
