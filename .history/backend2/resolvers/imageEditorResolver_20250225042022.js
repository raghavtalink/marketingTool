...
const processUpload = async (imageData) => {
  try {
    // Check if we're receiving a base64 string
    if (typeof imageData === 'string') {
      // Fix common base64 format issues
      const cleanedData = imageData
        .replace(/^data:image\/[a-z]+;base64;/, 'data:image/jpeg;base64,') // Fix double semicolon
        .replace(/^data:image\/[a-z]+,base64,/, 'data:image/jpeg;base64,'); // Fix wrong separator
      ...
      console.log('[processUpload] Successfully parsed base64 data');
  ...
  Mutation: {
    removeBackground: async (_, { input }, { user }) => {
      ...
      try {
        ...
        console.log(`[removeBackground] Product found: ${product.name}`);
  
        const imageBuffer = await processUpload(input.image);
        console.log(`[removeBackground] Image processed: ${imageBuffer.length} bytes`);
  
        // Configure background removal
        const config = {
          debug: true, // Enable debug for more info
          model: 'medium', // or 'fast' or 'best'
          output: {
            format: 'png',
            quality: 0.8,
            type: 'foreground'
        ...
        console.log('[removeBackground] Attempting background removal...');
