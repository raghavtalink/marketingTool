...
const { callLlama3 } = require('../utils/ai');
...
const findCompetitors = async (product) => {
  ...
  try {
    const response = await callLlama3(searchPrompt);
    const competitorData = JSON.parse(response.trim());
...
const marketAnalysisResolver = {
  ...
  Mutation: {
    ...
    suggestPricing: async (_, { input }, { user }) => {
      ...
      const allCompetitorPrices = [...competitors.map(c => c.price), ...input.competitorPrices];
      ...
      const prompt = `
        ...
        Competitor Prices: ${JSON.stringify(competitors, null, 2)}
