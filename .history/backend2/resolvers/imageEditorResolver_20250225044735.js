  ...
  Mutation: {
    removeBackground: async (_, { input }, { user }) => {
      try {
        ...
        // Get the correct public path using the package's default location
        const publicPath = `file://${path.resolve(
          path.join(__dirname, '../../node_modules/@imgly/background-removal-node/dist/')
        // Dynamic config based on image size
        const config = {
          debug: true,
          model: imageBuffer.length > 5 * 1024 * 1024 ? 'fast' : 'medium',
          publicPath: `file://${path.join(__dirname, 'node_modules/background-removal-node/dist/')}`,
          proxyToWorker: true,
          fetchArgs: {},    
          output: {
