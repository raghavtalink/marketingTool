...
function extractFlipkartData($, url) {
  ...
  const price = $('._30jeq3._16Jk6d').text().trim() ||
               $('._30jeq3').text().trim() ||
               $('div._25b18c div._30jeq3').text().trim() ||
               'Price not available';
  ...
  $('img.q6DClP, img._396cs4, img._2r_T1I, div._3kidJX img').each((i, el) => {
    let src = $(el).attr('src');
  ...
  // Extract reviews
  const reviews = [];
  $('.t-ZTKy, .qwjRop, ._6K-7Co').each((i, el) => {
    if (i < 10) { // Limit to 10 reviews
  ...
  $('._14cfVK tr, ._3k-BhJ tr').each((i, el) => {
