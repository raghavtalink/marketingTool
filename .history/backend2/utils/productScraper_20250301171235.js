...
function extractFlipkartData($, url) {
  const title = $('.B_NuCI').text().trim() || $('h1.yhB1nd').text().trim();
  let price = $('._30jeq3').text().trim();
  if (!price || price.includes('Bank Offer')) {
    price = $('div[class*="_30jeq3"]').first().text().trim();
  }
  const description = $('._1mXcCf').text().trim() || $('._1AN87F').text().trim();
  const images = [];
  $('img[class*="q6DClP"], img[class*="_396cs4"]').each((i, el) => {
    const src = $(el).attr('src');
    if (src && !src.includes('data:image') && !images.includes(src)) {
      images.push(src.replace(/128\/128/g, '832/832'));
    }
  });
  const rating = $('._3LWZlK').first().text().trim();
  const reviews = $('.t-ZTKy').map((i, el) => $(el).text().trim()).get().filter(t => t.length > 10);
  const specifications = $('._14cfVK').text().trim() || $('div[class*="_3k-BhJ"]').text().trim();
  const features = $('._2418kt li, ._1133chaP li').map((i, el) => $(el).text().trim()).get().filter(t => t.length > 5);
  const category = $('._3GIHBu').text().trim() || $('a[class*="_2whKao"]').map((i, el) => $(el).text().trim()).get().join(' > ');
