...
function extractFlipkartData($, url) {
    ...
    // Extract title
    const titleSelectors = ['span.B_NuCI', 'h1.yhB1nd', 'div._4rR01T', 'span.VU-ZEz'];
    ...
    // Extract price
    const priceSelectors = ['div._30jeq3._16Jk6d', 'div._30jeq3', 'div._1vC4OE._3qQ9m1', 'span._2j_7lZ', 'div._25b18c > div'];
    ...
    // Extract description
    const descSelectors = ['div._1mXcCf.RmoJUa', 'div._1UhVsV', 'div._1mXcCf', 'p._2-N8zT', 'div._2o-xpa'];
    let description = 'Not available';
    for (const selector of descSelectors) {
      const elem = getText(selector);
      if (elem && elem.length > 20) { // Ensure it’s a meaningful description
        description = elem;
        break;
      }
    }
    ...
    // Extract images
    const imageSelectors = ['div._396cs4._2amPTt._3qGmMb img', 'img._396cs4', 'div.cxPAYn img', 'div._3kidJX img'];
    const images = [];
    for (const selector of imageSelectors) {
      $(selector).each((i, el) => {
        let src = $(el).attr('src');
        if (src && !src.includes('data:image')) {
          src = src.replace(/128\/128/g, '832/832'); // Enhance image resolution
          images.push(src);
        }
      });
    ...
    // Extract rating
    const ratingSelectors = ['div._3LWZlK', 'span._2_R_DZ', 'div._3LWZlK._1BLPMq', 'div._2d4LTz'];
    let rating = 'Not available';
    for (const selector of ratingSelectors) {
      const elem = getText(selector);
      if (elem && /^\d+(\.\d+)?$/.test(elem)) { // Check if it’s a valid rating
        rating = elem;
        break;
      }
    }
    ...
    const reviewsCountSelectors = ['span._2_R_DZ', 'div._3LWZlK + span', 'span._38sUEc'];
    ...
    // Extract reviews
    const reviewSelectors = ['div.t-ZTKy', 'div._6K-7Co', 'p._2-N8zT', 'div._27M-vq'];
    const reviews = [];
    ...
    const specs = {};
    $('div._3k-BhJ, div._1AtVbE').each((i, section) => {
      const sectionTitle = getText('.flxcaE, ._21Ahn-', section);
      const specsObj = {};
      $(section).find('tr._1s_Smc.row, div._2H87NU').each((j, row) => {
        const key = getText('td._1hKmbr, span._2vNptZ', row);
        const value = getText('td._2vZqPX, div._2vNptZ + div', row);
        if (key && value) {
          specsObj[key] = value;
        }
      });
    ...
    // Extract features
    const featureSelectors = ['div._2418kt li', 'ul._1xgFaf li', 'div._3k-BhJ li', 'div._22QfMC'];
    const features = [];
    ...
    const categorySelectors = ['div._2olh4r a', 'div._2whKao', 'span._2whKao', 'div._1Wliux a'];
    let category = 'Other';
