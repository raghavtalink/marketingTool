...
async def chat_with_product(chat: ChatHistory, current_user: dict = Depends(get_current_user)):
    try:
        ...
        conversation = "You are a helpful AI assistant for an e-commerce product. Provide accurate and relevant information about the product. Keep responses concise and well-formatted.\n\n"
        ...
        Please provide information based on the above product details.
        """
        ...
        async def generate():
            try:
                ...
                response_stream = ai.prompt(conversation, stream=True)
                accumulated_response = ""
                
                async for chunk in response_stream:
                    accumulated_response += chunk
                    # Format the chunk as SSE data
                    yield f"data: {json.dumps({'content': chunk, 'full_response': accumulated_response})}\n\n"
                    
                    # Optional: Add small delay to prevent overwhelming the client
                    await asyncio.sleep(0.05)
                
                # Store final chat history in database
