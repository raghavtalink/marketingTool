...
async def chat_with_product(
    chat: ChatHistory,  # Remove search_web from function parameters
    current_user: dict = Depends(get_current_user)
):
    try:
        ...
        web_data = ""
        # Print the received search_web value for debugging
        print(f"\n🔍 Search web parameter received as: {chat.search_web}")
        
        # Only proceed with search if search_web is explicitly True
        if chat.search_web is True and last_message:  # Access search_web from chat object
            try:
                ...
                if "compare" in last_message.lower():
                    ...
                    # Default search focused on the product
        ...
        # Enhance the conversation prompt with better context and instructions
