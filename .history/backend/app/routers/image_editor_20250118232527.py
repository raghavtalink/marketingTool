...
async def generate_background(prompt: ImageGenerationPrompt, current_user: dict = Depends(get_current_user)):
    try:
        ...
        # Extract the base64 string properly
        if 'result' in result:
            # Remove any string formatting and get raw base64
            base64_str = result['result']
            if isinstance(base64_str, str):
                # Clean up the string if it's wrapped in quotes or has extra formatting
                base64_str = base64_str.strip("'").strip('"')
                if isinstance(base64_str, dict) and 'image' in base64_str:
                    base64_str = base64_str['image']
                return {"image": base64_str}
                
        raise HTTPException(
            status_code=500,
            detail="Invalid response format from image generation service"
        )
        
    except Exception as e:
        print(f"Error in generate_background: {str(e)}")  # Debug print
