...
def call_llama3(prompt):
    ...
    data = response.json()
    return {"message": data.get("result", {}).get("response", "No response generated.")}
    ...
    try:
        ...
        if search_web and last_message:
            try:
                ...
                web_data = google_search(last_message)
                print("\n🔹 Extracted Web Data 🔹")
                print(web_data)
            except Exception as e:
                print(f"Web search error: {str(e)}")
                # Continue without web data if search fails
                pass

        conversation = """You are a helpful AI assistant for an e-commerce product. 
        Provide accurate and relevant information about the product.
        ...
        # Use the modified call_llama3 function
        response = call_llama3(conversation, web_data if search_web else "")
        
        return JSONResponse({
            "content": response if isinstance(response, str) else response.get("message", ""),
            "format": "html",
            "web_data_used": bool(web_data)  # Indicate if web data was used
        })
