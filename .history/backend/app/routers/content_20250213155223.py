...
async def generate_content(prompt: AIPrompt, current_user: dict = Depends(get_current_user)):
    try:
        ...
        if prompt.prompt_type == "full_listing":
            ...
            ai_prompt = f"""
            You are an expert product marketer. Create a compelling and SEO-optimized title for the following product, using current market trends and popular keywords:
