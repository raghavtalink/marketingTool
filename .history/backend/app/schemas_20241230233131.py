...
class MarketTrendAnalysis(BaseModel):
    product_id: str
    trend_type: str  # 'market_position', 'competitor_analysis', 'demand_forecast'
    timeframe: str = 'current'  # 'current', 'weekly', 'monthly'

class DynamicPricing(BaseModel):
    product_id: str
    target_margin: float = Field(..., description="Target profit margin percentage")
    competitor_prices: List[float] = Field(default_factory=list, description="Known competitor prices")
    market_demand: str = Field(..., description="Current market demand level: low, normal, high")
    season: str = Field(..., description="Target season for pricing")
