...
def search_duckduckgo(query):
    ...
    # Look for any link whose href contains "uddg="
    links = soup.find_all("a", href=True)
    for link in links:
        href = link.get("href")
        if "uddg=" in href:
            # Extract the real URL using regex
            match = re.search(r"uddg=(https?://[^&]+)", href)
            if match:
                actual_url = requests.utils.unquote(match.group(1))
                print(f"DEBUG: Found URL: {actual_url}")  # Debug print for the URL
...
def scrape_website(url):
    ...
    if response.status_code != 200:
        return f"Error: Unable to retrieve webpage content. (Status code: {response.status_code})"
...
def ask_cloudflare_ai(user_input, search_query=None, url=None):
    ...
    # Build the payload by appending the scraped web_info to the user_input.
    payload = {
        "messages": [
