...
const BundleBuilder = () => {
  ...
  const getSuggestedCombinations = () => {
    ...
    // Calculate the number of combinations to show based on product count
    const productCount = products.length;
    let maxCombinations = 4; // Default limit
    
    if (productCount <= 5) {
      // For small catalogs, show all possible 2-product combinations
      for (let i = 0; i < productCount; i++) {
        for (let j = i + 1; j < productCount; j++) {
          combinations.push([products[i], products[j]]);
          if (combinations.length >= 10) break; // Safety limit
        }
        if (combinations.length >= 10) break; // Safety limit
      }
    } else if (productCount <= 10) {
      // For medium catalogs, mix same-category pairs and some random pairs
      
      // First, create category-based combinations
      const categoriesMap = {};
      ...
      // Add same-category pairs
      Object.values(categoriesMap).forEach(categoryProducts => {
        if (categoryProducts.length >= 2) {
          for (let i = 0; i < Math.min(categoryProducts.length - 1, 2); i++) {
            combinations.push([categoryProducts[i], categoryProducts[i + 1]]);
          }
        }
      });
      
      // Add some cross-category pairs
      const categories = Object.keys(categoriesMap);
      if (categories.length >= 2) {
        for (let i = 0; i < Math.min(categories.length, 2); i++) {
          const cat1 = categories[i];
          const cat2 = categories[(i + 1) % categories.length];
          if (categoriesMap[cat1].length > 0 && categoriesMap[cat2].length > 0) {
            combinations.push([categoriesMap[cat1][0], categoriesMap[cat2][0]]);
          }
        }
      }
      
      // Add some price-based combinations (low with high)
      const sortedByPrice = [...products].sort((a, b) => a.price - b.price);
      if (sortedByPrice.length >= 2) {
        combinations.push([sortedByPrice[0], sortedByPrice[sortedByPrice.length - 1]]);
      }
    } else {
      // For large catalogs, generate diverse combinations
      maxCombinations = 6; // Show more options for large catalogs
      
      // Add category-based combinations
      const categoriesMap = {};
      products.forEach(product => {
        if (!categoriesMap[product.category]) {
          categoriesMap[product.category] = [];
      ...
      // Add same-category pairs
      Object.values(categoriesMap).forEach(categoryProducts => {
        if (categoryProducts.length >= 2 && combinations.length < maxCombinations) {
          combinations.push([categoryProducts[0], categoryProducts[1]]);
        }
      });
      
      // Add some best-sellers or high-inventory items
      // (In a real scenario, you might sort by sales data)
      const sortedByInventory = [...products].sort((a, b) => b.inventoryCount - a.inventoryCount);
      if (sortedByInventory.length >= 3 && combinations.length < maxCombinations) {
        combinations.push([sortedByInventory[0], sortedByInventory[1], sortedByInventory[2]]);
      }
      
      // Add some price-ranged combinations
      const sortedByPrice = [...products].sort((a, b) => a.price - b.price);
      if (sortedByPrice.length >= 4 && combinations.length < maxCombinations) {
        // Low-price bundle
        combinations.push([sortedByPrice[0], sortedByPrice[1]]);
        // Premium bundle
        combinations.push([sortedByPrice[sortedByPrice.length - 2], sortedByPrice[sortedByPrice.length - 1]]);
      }
      
      // Add some seasonal product combinations if possible
      // This would require season data which we don't have, so this is a placeholder
      // We'll add a random combination as a fallback
      if (combinations.length < maxCombinations) {
        const randomIndex1 = Math.floor(Math.random() * productCount);
        let randomIndex2;
        do {
          randomIndex2 = Math.floor(Math.random() * productCount);
        } while (randomIndex2 === randomIndex1);
        
        combinations.push([products[randomIndex1], products[randomIndex2]]);
      }
    }
    
    // Ensure we don't return more than maxCombinations
