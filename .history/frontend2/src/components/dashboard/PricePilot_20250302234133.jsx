...
const PricePilot = () => {
  ...
  const filteredProducts = !loading && data?.products 
          ...
          <div className="mb-6">
            ...
            <div className="relative mb-3">
              ...
              <input
                ...
                className="bg-gray-800 border border-gray-700 text-white rounded-lg pl-10 pr-4 py-2.5 w-full focus:ring-2 focus:ring-purple-500 focus:border-transparent"
              />
            </div>
            
            {/* Product cards */}
            <div className="overflow-y-auto max-h-[180px] rounded-lg border border-gray-700 bg-gray-800/50">
              {loading ? (
                ...
                filteredProducts.map(product => (
                  <div 
                    ...
                    className={`p-3 border-b border-gray-700 cursor-pointer transition-colors hover:bg-gray-700/50 ${
                      selectedProduct?.id === product.id ? 'bg-purple-900/20 border-l-4 border-l-purple-500' : ''
                    }`}
                  >
                    <div className="flex justify-between items-center">
                      <div>
                        <h3 className={`font-medium ${selectedProduct?.id === product.id ? 'text-purple-300' : 'text-white'}`}>
                          {product.name}
                        </h3>
                        <p className="text-sm text-gray-400">{product.category}</p>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-white">{product.currency} {product.price}</p>
            ...
            {selectedProduct && (
              <div className="mt-3 p-3 bg-gray-800/80 rounded-lg border border-purple-500/30">
                <div className="flex justify-between items-center">
                  <h3 className="font-medium text-purple-300">Selected: {selectedProduct.name}</h3>
                  <button 
                ...
                <div className="mt-3 sm:mt-0 flex items-center bg-gray-800/60 rounded-lg px-4 py-2 border border-gray-700">
                  ...
                  <span className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-green-400 to-emerald-500">
                    {pricingResult.priceRange 
                      ? `${pricingResult.currency || 'INR'} ${pricingResult.priceRange}`
                      : `${pricingResult.currency || 'INR'} ${pricingResult.targetMargin ? (350 * (1 + pricingResult.targetMargin/100)).toFixed(0) : '350-400'}`}
                  </span>
                </div>
              </div>
