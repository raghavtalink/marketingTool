...
const ListingWizard = () => {
        ...
        <div>
          ...
          <div className="flex justify-between mt-8">
            ...
            <button
              ...
              disabled={isLoading}
              className={`px-6 py-2 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-lg flex items-center ${
  ...
  const renderResults = () => {
    ...
    const renderContent = () => {
      ...
      if (generatedContent.contentType === 'seo_tags') {
        ...
        const htmlContent = extractHtmlContent(generatedContent.content);
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="absolute top-2 right-2 flex space-x-2">
              ...
              {hasHtml && (
                <button 
                  onClick={() => handleCopy(htmlContent, 'html-content')}
                  className="p-1.5 bg-gray-700 rounded-md hover:bg-gray-600 transition-colors flex items-center"
                  title="Copy HTML only"
                >
                  {copiedSection === 'html-content' ? <Check size={16} className="mr-1" /> : <Code size={16} className="mr-1" />}
            ...
            {/* HTML Preview with better styling */}
            {hasHtml && (
            ...
            {hasHtml && (
              <div className="mt-6">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <button 
                    onClick={() => handleCopy(htmlContent, 'html-code')}
                    className="p-1 bg-gray-700 rounded-md hover:bg-gray-600 transition-colors"
                    title="Copy HTML code"
                  >
                    {copiedSection === 'html-code' ? <Check size={14} /> : <Copy size={14} />}
  ...
  // Add this helper function to extract HTML content
  const extractHtmlContent = (content) => {
    // Try to extract HTML tags from the content
    const htmlRegex = /<[^>]+>[^<]*<\/[^>]+>|<[^/>]+\/>/g;
    ...
    // If no HTML tags found, check for code blocks
